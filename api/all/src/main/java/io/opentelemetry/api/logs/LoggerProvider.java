...
/**
 * A registry for creating named {@link Logger}s. The name <i>Provider</i> is for consistency with
...
public interface LoggerProvider {
  ...
  /**
   * Returns a no-op {@link LoggerProvider} which only creates no-op {@link LogRecord}s which do not
  ...
  /**
   * Gets or creates a named and versioned Logger instance with instrumentation scope attribute
   * <i>event.domain</i> set to <i>eventDomain</i>.
   *
   * @param instrumentationScopeName A name uniquely identifying the instrumentation scope, such as
   *     the instrumentation library, package, or fully qualified class name. Must not be null.
   * @param instrumentationScopeVersion The version of the instrumentation scope (e.g., "1.0.0").
   * @param eventDomain The domain for the events created by the logger instance.
   * @return an Logger instance.
   */
  Logger get(
